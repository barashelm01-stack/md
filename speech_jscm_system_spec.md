功率归一化采用三级策略实现。第一，在 Conformer-JSCM 模型内部，encoder 输出复数信道符号 z 后，按每个语音 segment、每个 batch 样本独立做复符号平均功率归一化，即令 mean(I^2 + Q^2) = 1.0，归一化范围是整个 segment，不能按 radio frame 单独归一化；这样训练、仿真和推理中的 JSCM 符号功率定义保持一致。第二，JSCM 符号在发送到 FM 调制前，需要先将 complex symbols 按 [Re0, Im0, Re1, Im1, ...] 串行化为 float slots，然后使用固定缩放系数 alpha 映射到 FM 输入范围，例如 u = clip(alpha * slots, -1.0, 1.0)，初始 alpha = 0.30，并记录 clip rate；接收端 FM 解调后应利用每帧 pilot 估计幅度增益，再除以该增益和 alpha 恢复到 JSCM decoder 所需的符号尺度。第三，在 Opus/GFSK 和 JSCM/FM 两条链路中，调制后进入 USRP Sink 前的 complex baseband 都要统一 RMS 标定，要求调制输出的平均功率一致，然后统一乘以同一个 tx_amp；SNR sweep 只允许调整 tx_gain 和 tx_amp，不能改变 JSCM 的模型内 power normalization、不能改变 alpha，也不能改变接收端 rx_gain。实现时需要在日志中记录 mean_power_before_norm、mean_power_after_norm、alpha、clip_rate、tx_amp、tx_gain、tx_baseband_rms、rx_iq_rms 和 pilot_gain，用于检查是否存在削顶、功率不公平或尺度恢复错误。
channel_mapper 采用轻量线性投影结构，不要设计成复杂解码器。Conformer encoder 输出隐状态 h 后，先经过 LayerNorm，再经过一个两层 MLP 将特征映射到固定数量的复数信道符号：h -> LayerNorm -> Linear(d_model, ffn_dim) -> GELU -> Dropout -> Linear(ffn_dim, 2 * num_complex_symbols)，最后 reshape 为 [B, num_complex_symbols, 2]，其中最后一维表示 I/Q，即 real/imag。num_complex_symbols 由码率配置决定，例如 low/mid/high 分别为 784/1568/3136。channel_mapper 只负责把语音特征压缩成连续复数符号，不在内部做功率归一化、不做量化、不做信道噪声注入；功率归一化必须由后续独立的 ComplexPowerNorm 完成。为了保证不同码率可切换，建议将 num_complex_symbols 作为配置参数传入，而不是写死在模型里。
在 JSCM + FM 链路中，CFO 校正应放在 USRP Source 输出之后、FM Demod/Quadrature Demod 之前，也就是对接收端的 complex IQ 基带信号先做频偏估计与频率平移，再进入 FM 解调。具体流程为：USRP Source -> DC Blocker -> optional LPF -> CFO estimation using preamble/pilot -> Frequency Xlating FIR Filter or complex mixer CFO correction -> LPF -> FM Demod -> float frame synchronization -> pilot gain correction -> alpha inverse mapping -> float slots to complex symbols -> JSCM decoder。不能把 CFO 校正放在 FM Demod 之后，因为 FM 解调会把载波频偏表现为 float 输出中的直流偏置或慢变趋势，此时再校正已经会污染 JSCM payload。实现时应优先用每个 burst/frame 的已知 preamble 或 pilot 估计 estimated_cfo_hz，然后对原始 IQ 乘以 exp(-j*2*pi*estimated_cfo_hz*n/fs) 或用 GNU Radio 的 Frequency Xlating FIR Filter 完成频移；校正后的 IQ 再进入 FM 解调。每次 run 需要记录 estimated_cfo_hz、pilot_corr_peak、timing_offset_samples 和校正前后的 pilot SNR，用于判断 CFO 校正是否有效。
